...
from Barcode import InventoryLog
...
class IMS:
    def __init__(self, root):
        ...
        title = Label(self.root, text="KLBN Corp. Inventory Management System", image=self.icon_title, compound=LEFT, font=("times new roman", 40, "bold"), bg="#010c48", fg="white", anchor="w", padx=20).place(x=0, y=0, relwidth=1, height=80)
        ...
        btn_category = Button(LeftMenu, text="Category", command=self.category, image=self.icon_side, compound=LEFT, padx=5, anchor="w",
                              font=("times new roman", 20, "bold"), bg="white", bd=3, cursor="hand2").pack(side=TOP,
                                                                                                           fill=X)
        btn_inventory = Button(LeftMenu, text="Inventory Log", command=self.inventory, image=self.icon_side, compound=LEFT, padx=5, anchor="w",
                               font=("times new roman", 20, "bold"), bg="white", bd=3, cursor="hand2").pack(side=TOP,
                                                                                                           fill=X)
...
# ----------------------------------------------------------
    ...
    def inventory(self):
        self.new_win = Toplevel(self.root)
        self.new_obj = InventoryLog(self.new_win)

    def sales(self):
        self.new_win = Toplevel(self.root)
        self.new_obj = SalesReport(self.new_win)
